...
export class HiveSectionListComponent implements OnInit {
  ...
  onDelete(hiveSectionId: number) {
    this.setDeletedStatus(hiveSectionId, true);
  }

  onRestore(hiveSectionId: number) {
    this.setDeletedStatus(hiveSectionId, false);
  }

  private setDeletedStatus(hiveSectionId: number, newDeletedStatus: boolean) {
    var hiveSection = this.hiveSections.find(h => h.id == hiveSectionId);
    this.hiveSectionService.setHiveSectionStatus(hiveSectionId, newDeletedStatus).subscribe(c => hiveSection.isDeleted = newDeletedStatus);
  }
}
